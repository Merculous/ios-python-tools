...
class TSS:
    def __init__(self, device, version, ecid):
        super().__init__()
        self.device = device
    ...
    def saveblobs(self, device, ecid):
        stuff = APIParser(self.device, self.version)
        signed_versions = stuff.signed(self.device)
        ...
        for version, buildid in signed_versions:
            ...
            if os.path.exists(manifest):
                ...
                stuff.linksForDevice(self.device)
    ...
    def saveBlobsWithTSSChecker(self):
        stuff = APIParser(self.device, self.version)
        devices = {
            "iPhone3,1": [str(806884279284), stuff.signed("iPhone3,1")],
            "iPhone3,3": [str(2902611023180), stuff.signed("iPhone3,3")],
            # "iPhone4,1": [str(2271501886349), ipswapi.signed("iPhone4,1")], locked
            "iPhone4,1": [str(2622681783562), stuff.signed("iPhone4,1")],
            "iPhone5,1": [str(3312880722830), stuff.signed("iPhone5,1")],
            "iPhone5,2": [str(2760926895370), stuff.signed("iPhone5,2")],
            "iPhone5,3": [str(2781026528712), stuff.signed("iPhone5,3")],
            "iPhone9,4": [str(5683998923293478), stuff.signed("iPhone9,4")]
        }
        ...
        for nonce in a7_dfu_nonces:
            ios = stuff.signed('iPhone6,1')[0][0]
        ...
        for nonce in a8_dfu_nonces:
            ios = stuff.signed('iPhone7,2')[0][0]
        ...
        ios = stuff.signed('iPhone11,6')[0][0]
